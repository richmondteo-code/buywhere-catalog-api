...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: "https://buywhere.ai/directory",
  },
};
        ...
        <section className="border-b border-slate-200 bg-white">
          <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 md:py-16">
            ...
            <p className="max-w-3xl text-lg leading-8 text-slate-600">
              Browse all platforms where BuyWhere is listed — MCP marketplaces, AI agent directories, API registries, and developer tool listings.
            </p>
          </div>
        </section>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {allContent.map((doc) => (
              <Link
                ...
                className="group overflow-hidden rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm transition-all hover:border-indigo-200 hover:shadow-md"
              >
                {doc!.category && (
                  <span className="mb-3 inline-flex rounded-full bg-indigo-50 px-3 py-1 text-xs font-medium text-indigo-700">
                    {doc!.category}
                  </span>
                )}
