...
const SEEDS = [
    {
        ...
        expertSummary: 'Compare prices for this electronics product across major Singapore retailers. Prices are updated daily from Lazada, Shopee, Best Denki, and others.',
    },
    {
        category: 'home',
        slugSuffix: 'home',
        expertSummary: 'Find the best deal on this home product in Singapore. Our price tracker monitors multiple retailers so you never overpay.',
    },
    {
        category: 'health',
        slugSuffix: 'health',
        expertSummary: 'Compare prices and availability for this health product across Singapore retailers. Updated daily.',
    },
];
async function seed() {
    for (const spec of SEEDS) {
        // Pick a product that has at least one retailer price and belongs to the category
        const productResult = await config_1.db.query(`SELECT p.id, p.name
       FROM products p
       ...
       ORDER BY p.id
       LIMIT 1`, [`%${spec.category}%`]);
        if (productResult.rows.length === 0) {
            // Fall back: pick any product with a price
            const fallback = await config_1.db.query(`SELECT p.id, p.name
         FROM products p
       ...
       RETURNING id, slug, status`, [cleanSlug, product.id, spec.category, spec.expertSummary]);
