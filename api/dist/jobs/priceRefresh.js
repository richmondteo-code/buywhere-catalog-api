...
async function callScraper(scraperUrl, productId) {
...
async function runPriceRefresh() {
    ...
    const pagesResult = await config_1.db.query(`SELECT product_id::text, slug
    ...
    await logRun(summary);
    console.log(`[price-refresh] Done — success: ${successes.length}, failed: ${failures.length} / ${pages.length}`);
