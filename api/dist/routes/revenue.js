...
// GET /v1/revenue/report
// ---------------------------------------------------------------------------
router.get('/report', apiKey_1.requireApiKey, async (req, res) => {
    ...
    try {
        const [kpiResult, coverageResult, weeklyResult, clicksResult, convResult] = await Promise.all([
         ...
         FROM (SELECT 1) AS _dummy`, [days]),
            ...
            config_1.db.query(`WITH retailer_products AS (
         ...
         ORDER BY SUM(rp.total_products) DESC`),
            ...
            config_1.db.query(`WITH weeks AS (
         ...
         ORDER BY w.week_start DESC`, [weeks]),
        ...
        const totalProducts = coverageResult.rows.reduce((s, r) => s + parseInt(r.total_products), 0);
        const totalLinked = coverageResult.rows.reduce((s, r) => s + parseInt(r.products_with_links), 0);
        // -----------------------------------------------------------------------
        ...
        if (parseInt(kpi.total_clicks) === 0) {
            gaps.push({
                ...
                recommendation: 'Populate affiliate_links for at least one merchant and verify /r/:slug/:productId redirect route logs to affiliate_clicks.',
            });
        }
        if (parseInt(kpi.total_conversions) === 0) {
            gaps.push({
                ...
                impact: 'Revenue is $0. No completed purchases have been attributed — either no affiliate network is posting postbacks, or no clicks have converted.',
                recommendation: 'Integrate an affiliate network postback webhook (e.g. Commission Factory, Impact, ShareASale) that writes to conversions + commission_decisions tables.',
            });
        }
        if (totalLinked === 0) {
            gaps.push({
                ...
                recommendation: 'For each merchant (amazon.sg, challenger.sg, fairprice.com.sg, …), obtain affiliate programme credentials and bulk-insert affiliate_links rows mapping product IDs to their affiliate tracking URLs.',
            });
        }
        res.json({
            meta: {
