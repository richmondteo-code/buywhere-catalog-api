...
router.get('/:slug', async (req, res) => {
    ...
    try {
        [productsResult, subCatsResult] = await Promise.all([
            config_1.db.query(`SELECT id, name AS title, price, currency, image_url, platform::text AS domain, product_url AS url
         FROM products
         WHERE currency = 'SGD' AND category_path[1] = $1
         ORDER BY updated_at DESC LIMIT $2`, [categoryName, limit]),
            config_1.db.query(`SELECT category_path[2] AS sub_name, COUNT(*) AS cnt
         FROM products
         WHERE currency = 'SGD' AND category_path[1] = $1
           AND array_length(category_path, 1) > 1
         GROUP BY category_path[2]
         ORDER BY cnt DESC LIMIT 10`, [categoryName]),
        ]);
    }
    catch (_) {
    ...
    const subCatHtml = subCats.length
        ? `<div style="margin:10px 0">${subCats
            .filter(s => s.sub_name)
            .map(s => `<span class="subcat"><a href="/c/${encodeURIComponent(s.sub_name.toLowerCase().replace(/[^a-z0-9]+/g, '-'))}">${escHtml(s.sub_name)}</a> (${s.cnt})</span>`)
            .join('')}</div>`
        : '';
