...
router.get('/guides/mcp', (_req: Request, res: Response) => {
...
<p><strong>Transport:</strong> HTTP (<code>POST ${API_BASE_URL}/mcp</code>) for remote agents. STDIO (local process) coming soon via npm.</p>

<h2>Install</h2>
<p><strong>The hosted MCP server is live.</strong> Point your MCP client directly at <code>${API_BASE_URL}/mcp</code> — no local install required.</p>
<div class="callout">
...
<pre><code>POST ${API_BASE_URL}/mcp
...
<p>Pass your API key as a Bearer token. Get a free key at <code>POST ${API_BASE_URL}/v1/auth/register</code>.</p>
<table>
...
<p style="margin-top:3em;color:#6b7280;font-size:.85em">
  <a href="${API_BASE_URL}/openapi.json">OpenAPI spec</a> ·
  <a href="${API_BASE_URL}/.well-known/ai-plugin.json">Plugin manifest</a> ·
